    ...
    for i = 1, #entities do
        ...
        if(ent.Type == 1000 and ent.Variant == 981 and ent.SubType == 0)then
            for j = 1, #entities do
                ...
                ( (entj.Position - ent.Position):Length() < ent.Size * 12 ) then
                    local veloEase = math.max((entj.Velocity:Length())^0.35, 1)
                    entj:AddVelocity((ent.Position - entj.Position):Normalized() / 2 * veloEase)
                    if (entj.Velocity:Length() >= 20) then
						entj.Velocity = entj.Velocity / 2
                    end

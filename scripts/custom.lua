    ...
    if player:HasCollectible(mys_item) then
		for i = 0, rooms.Size - 1 do
            ...
            if (roomType == RoomType.ROOM_SHOP or
                roomType == RoomType.ROOM_TREASURE or
                ...
                roomType == RoomType.ROOM_BOSSRUSH or
                roomType == RoomType.ROOM_ISAACS or
                roomType == RoomType.ROOM_BARREN or
                roomType == RoomType.ROOM_CHEST or
            roomType == RoomType.ROOM_DICE) then
                level:GetRoomByIdx(roomIndex).DisplayFlags = level:GetRoomByIdx(roomIndex).DisplayFlags | 1
            end
        end
        level:UpdateVisibility()
    end
